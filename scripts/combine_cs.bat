...
rem 결과 파일 전체 경로 설정
set "outputFile=%destinationFolder%\combined_scripts.txt"

rem 파라미터가 없으면 오류 메시지 후 종료
if "%destinationFolder%"=="" (
    echo [오류] 결과물 저장 경로가 전달되지 않았습니다.
    echo 사용법: combine_cs.bat [결과폴더] [분석폴더1] [분석폴더2] ...
    pause
    exit /b 1
)
...
echo.
echo =======================================================
echo C# 스크립트 병합을 시작합니다 (하위 폴더 포함).
...
for /r "%~1" %%f in (*.cs) do (
    ...
    if errorlevel 1 (
        ...
        echo "[발견] %%~nxf 파일 처리 중..."
        echo. >> "%outputFile%"
        echo --- 파일 시작: %%f --- >> "%outputFile%"
        type "%%f" >> "%outputFile%"
        echo. >> "%outputFile%"
        echo --- 파일 끝: %%~nxf --- >> "%outputFile%"
        set /a fileCount+=1
    ) else (
...
echo =======================================================
if %fileCount% gtr 0 (
    echo [성공] 총 %fileCount%개의 .cs 파일 내용이 아래 파일로 저장되었습니다.
    echo "%outputFile%"
) else (
